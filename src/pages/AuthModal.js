...
const AuthModal = ({ setUserData }) => {
  ...
  const handleLogin = async () => {
    try {
      const response = await axios.post('https://backend.bytebay.ru/api/auth/local', {
        identifier: email,
        password: password,
      });
      if (response.data && response.data.user && response.data.jwt) {
        ...
        const userDataResponse = await axios.get(`https://backend.bytebay.ru/api/users/${response.data.user.id}?populate=*`);
  ...
  const handleRegister = async () => {
    try {
      const response = await axios.post('https://backend.bytebay.ru/api/auth/local/register', {
        username: username,
        email: email,
        password: password,
      });
