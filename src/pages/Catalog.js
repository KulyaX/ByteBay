  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const methodUploadsResponse = await axios.get('http://localhost:1337/api/method-uploads?populate=*');
        setMethodUploads(methodUploadsResponse.data.data);

        // Fetch degree completions
        const degreeCompletionsResponse = await axios.get('http://localhost:1337/api/degree-completions?populate=*');
        setDegreeCompletions(degreeCompletionsResponse.data.data);

        // Fetch products data
        const productsResponse = await axios.get('http://localhost:1337/api/products?populate=*&filters[statePublication][$eq]=true');
