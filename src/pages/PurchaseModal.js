...
const PurchaseModal = ({ visible, onClose, product, licenseType, userId }) => {
  ...
  const fetchPlatformParams = async () => {
    try {
      const response = await axios.get('http://localhost:1337/api/platform-params?populate=*');
  ...
  const fetchRequisites = async () => {
    try {
      const response = await axios.get('http://localhost:1337/api/requisites?populate=*');
  ...
  const handlePurchase = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:1337/api/orders', newOrder, {
        headers: {
  ...
  const handleConfirmByUser = async () => {
    try {
      ...
      const responsePut = await axios.put(`http://localhost:1337/api/orders/${order.data.id}`, newOrderData, {
        headers: {
          Authorization: `Bearer ${jwt}`
        }
      });

      const responseGet = await axios.get(`http://localhost:1337/api/orders/${order.data.id}?populate=*`);
  ...
  const handleReloadStatus = async () => {
    try {
      const response = await axios.get(`http://localhost:1337/api/orders/${order.data.id}?populate=*`);
      ...
      if (statusOrder.attributes.value === 'paid') {
        ...
        await axios.put(`http://localhost:1337/api/products/${product.data.id}`, {
          data: {
  ...
  const handleCommentSubmit = async () => {
    try {
      ...
      const responseReviews = await axios.get(`http://localhost:1337/api/reviews?filters[product][id]=${product.data.id}`);
