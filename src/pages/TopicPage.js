...
const TopicPage = () => {
  ...
  useEffect(() => {
    const fetchTopicAndMessages = async () => {
      try {
        const topicResponse = await axios.get(`http://localhost:1337/api/topics/${id}?populate=*`);
        setTopic(topicResponse.data);

        const messagesResponse = await axios.get(`http://localhost:1337/api/forum-messages?populate=*&filters[topic][id]=${id}`);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:1337/api/forum-messages', {
        data: {
